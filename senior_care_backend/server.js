...
app.delete('/api/users/:userId', async (req, res) => {
    try {
        ...
        console.log(`🗑️ DELETING ACCOUNT: User ${userId}`);

        // Use SERVICE_ROLE to bypass RLS for cascade delete
        const supabaseServiceRole = createClient(
            process.env.SUPABASE_URL,
            process.env.SUPABASE_SERVICE_ROLE_KEY
        );
        ...
        const { error: familyConnError } = await supabaseServiceRole
            .from('family_connections')
            .delete()
        ...
        console.log('  ✅ Family connections (senior) deleted');

        // 5. Delete family connections (where user is FAMILY MEMBER)
        console.log(`  → Deleting family_connections where user is family member`);
        const { error: familyConnError2 } = await supabaseServiceRole
            .from('family_connections')
            .delete()
        ...
        console.log('  ✅ Family connections (family member) deleted');
        ...
        console.log(`✅ ACCOUNT COMPLETELY DELETED: User ${userId}`);
              ...
              <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #fff3e0; border-radius: 8px;">
                ...
                <div style="background: white; padding: 20px; border-left: 4px solid #ff6f00; margin: 20px 0; border-radius: 4px;">
                  ...
                  <p style="margin: 15px 0 0 0; color: #666; font-size: 14px;">
                    Scheduled for: <strong>${new Date(new Date(notification.scheduled_time).getTime() + notification.reminder_minutes * 60000).toLocaleString('en-US', {timeZone: senior.timezone || 'America/Denver', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit'})}</strong>
                  </p>
                </div>
